...
{
    //SparsePlayerReplicationInfo needs to be able to support both Controller and PlayerReplicationInfo being the owner of it.
    if (Owner != None)
    {
        if (Controller(SparseOwningActor) != None)
        {
            SparseOwningActor = Controller(SparseOwningActor).PlayerReplicationInfo;
        }
        else
        {
